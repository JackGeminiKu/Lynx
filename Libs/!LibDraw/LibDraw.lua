...
function LibDraw.Draw2DLine(sx, sy, ex, ey)
    ...
    if not WorldToScreen or not sx or not sy or not ex or not ey then
        return
    end
...
function LibDraw.Texture(config, x, y, z, alphaA)
    ...
    if not WorldToScreen or not texture or not width or not height or not x or not y or not z then
        return
    end
